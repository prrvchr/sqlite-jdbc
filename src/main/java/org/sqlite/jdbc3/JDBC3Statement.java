...
public abstract class JDBC3Statement extends CoreStatement {
    ...
    protected boolean executeStatementInsert(String sql1, String keys) throws SQLException {
        if (QueryUtils.isInsertQuery(sql1)) {
            String sql2 = new String(sql1);
            sql2 = QueryUtils.addReturningClause(sql1, keys);
            if (sql2.length() != sql1.length()) {
                updateGeneratedKeys(executeQuery(sql2));
                return true;
            }
        }
        return false;
    ...
    public int executeUpdate(String sql, String[] colnames) throws SQLException {
        if (executeStatementInsert(sql, String.join(",", colnames))) {
            return 1;
        }
    ...
    public long executeLargeUpdate(String sql, String[] colnames) throws SQLException {
        if (executeStatementInsert(sql, String.join(",", colnames))) {
            return 1;
        }
        return executeLargeUpdate(sql);
